...
matrix diff(double t, const matrix &Y, matrix P)
{
	...
	matrix dY(3);
	double a = 0.98, b = 0.63, g = 9.81, PA = 1.0, PB = 1.0, // DA nie jest sta³¹, teraz próbujemy znaleŸæ œrednicê otworu zbiornika A
	...
	dY(1) = FAout + Fin - FBout;
	dY(2) = Fin / Y(1)*(Tin - Y(2)) + FAout / Y(1)*(TA - Y(2));
#elif LAB_NO==2 && LAB_PART==2 && TASK == 5 //policzenie wartoœci z LAB_NO = 1 dla nowego DA
	matrix dY(3);
	double a = 0.98, b = 0.63, g = 9.81, PA = 1.0, PB = 1.0,
